...
subplot(3, 2, 1), plot(x(Pty0in), zeros(size(x(Pty0in))), 'bo', 'markers',...
    MarkerSize,'MarkerFaceColor', 'b');
hold on
subplot(3, 2, 1), plot(x(Pty1in), zeros(size(x(Pty1in))), 'go', 'markers',...
    MarkerSize,'MarkerFaceColor', 'g');
subplot(3, 2, 1),quiver(x, zeros(size(x)), Fx * ScaleF, zeros(size(Fx)) * ScaleF, 0, 'm', 'linewidth', 2);
hold off
axis(Limits);
...
subplot(3,2,3), plot(x(Pty0in), zeros(size(x(Pty0in))), 'bo', 'markers',...
    MarkerSize,'MarkerFaceColor','b');
hold on
subplot(3,2,3), plot(x(Pty1in), zeros(size(x(Pty1in))), 'go', 'markers',...
    MarkerSize,'MarkerFaceColor','g');
subplot(3,2,3),quiver(x, zeros(size(x)), Vx * ScaleV, zeros(size(Vx)) * ScaleV, 0, 'r', 'linewidth', 2);
hold off
axis(Limits);
...
% AE = Phi;
subplot(3, 2, 4),scatter3(x,zeros(size(x)),AE,ones(1,nAtoms)*300,AE,'fill');
axis(Limits);
view(2);
title('Atom Energy (KE + Pot)')

subplot(3, 2, 5), plot(x(Pty0in), zeros(size(x(Pty0in))), 'bo', 'markers',...
    MarkerSize, 'MarkerFaceColor', 'b');
hold on
subplot(3, 2, 5), plot(x(Pty1in), zeros(size(x(Pty1in))), 'go', 'markers',...
    MarkerSize, 'MarkerFaceColor', 'g');
hold off
title('Atoms (All)')
xlabel('X')
